...
vec fourier_motzkin (const mat& A, const double tol=1e-6, const double the_best_number = 1) {
	...
	if (A.n_cols == 1) {
		if (upper_bounds.n_elem > 0) throw logic_error("system is infeasible");
		else return vec();
	}
	...
	vec smaller_x(A.n_cols-2, fill::zeros);
	if (smaller_A.n_rows > 0) {
		smaller_x = fourier_motzkin(smaller_A);
	}
